...
std::vector<std::pair<int, gtsam::Pose3>> localization_result_vec;
...
void OnSubscribeLocalizationResult(const mobinn_nav_msgs::AnswerLocalization::ConstPtr & msg) {
    ...
    if(!initialPositionFound) {
        ...
        std::pair<int, gtsam::Pose3> priorLocValue;
        priorLocValue.first = query_point_id;
        priorLocValue.second = locPose;
        mtx.lock();
        localization_result_vec.push_back(priorLocValue);
        mtx.unlock();
...
void GraphParamInitialization() {
    ...
    loopPoseNoise  = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 10e-2, 10e-2, 10e-2, 10e-2, 10e-2, 10e-2).finished()); // rad,rad,rad,m, m, m
    priorPoseNoise  = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2).finished()); // rad,rad,rad,m, m, m
    priorVelNoise   = gtsam::noiseModel::Isotropic::Sigma(3, 1e4); // m/s
    priorBiasNoise  = gtsam::noiseModel::Isotropic::Sigma(6, 100e-3); // 1e-2 ~ 1e-3 seems to be good
...
void OnSubscribeFeaturePoints(const mobinn_nav_msgs::FeaturePoints::ConstPtr & msg) {
    ...
    if(!initialPositionFound) {
        ...
        //Community Center Front Column

        // pose.position.x = 3.870117572068245;
        // pose.position.y = 93.50175131324595;
        // pose.position.z = 0.5539076286012515;
        // pose.orientation.x = -0.0022235317845449858;
        // pose.orientation.y = -0.008184333665297465;
        // pose.orientation.z = 0.7346102439150937;
        // pose.orientation.w = 0.6784363360304321;
    ...
    if(time_diff > 1.0 || distance_diff > 0.5 || angle_diff > 10 * M_PI/180.0) {
        ...
        if(haveSuccessiveMatch) {
            ...
            mtx.lock();
            for(int i = 0; i<localization_result_vec.size(); ++i) {
                gtsam::PriorFactor<gtsam::Pose3> priorLocPose(gtsam::Symbol('x', localization_result_vec[i].first), localization_result_vec[i].second, loopPoseNoise);
                factorGraph.add(priorLocPose);
            }
            localization_result_vec.clear();
            mtx.unlock();
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "slam_module_cloud_init", ros::init_options::NoSigintHandler);
